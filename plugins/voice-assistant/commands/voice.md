# Voice Assistant (/voice)

Voice input and output for Claude Code. Record voice, transcribe, and get spoken responses.

## Allowed Tools

Bash, Read, Write, AskUserQuestion

## Usage

- `/voice` - Show voice assistant status and options
- `/voice ask` - Record voice and ask Claude (STT → Claude → TTS)
- `/voice on` - Enable TTS (speak responses)
- `/voice off` - Disable TTS
- `/voice config` - Show current configuration

## Workflow

### /voice ask

1. **Record**: Use `${pluginDir}/.venv/bin/python ${pluginDir}/scripts/record.py` to record voice
   - Inform user: "Recording... Press Ctrl+C when done (max 30s)"
   - Wait for recording to complete

2. **Transcribe**: Use `${pluginDir}/.venv/bin/python ${pluginDir}/scripts/transcribe.py` to convert speech to text
   - Display transcribed text to user

3. **Confirm**: Ask user if the transcription is correct
   - Options: "Yes, ask this", "Re-record", "Cancel"

4. **Process**: If confirmed, treat the transcribed text as user input
   - Claude will respond normally
   - TTS hook will speak the response (if enabled)

### /voice on/off

Toggle TTS by editing `${pluginDir}/config.json`:
- Set `tts.enabled` to `true` or `false`
- Confirm the change to user

### /voice config

Display current settings from `${pluginDir}/config.json`:
- STT provider (whisper-cpp or openai)
- TTS mode (summary or full)
- Voice settings

## Dependencies

- **sox**: Audio recording (`brew install sox`)
- **whisper-cpp**: Local STT (`brew install whisper-cpp`) - or OpenAI API
- **anthropic**: TTS summarization (`pip install anthropic`)

## Reference

See `skills/voice-assistant/SKILL.md` for detailed workflow specification.
